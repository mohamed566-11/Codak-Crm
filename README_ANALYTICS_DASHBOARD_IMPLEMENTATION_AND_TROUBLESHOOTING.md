...
هذا المستند يعتبر **دليلاً مرجعياً كاملاً ومفصلاً 100% دون أي اختصار أو حذف للأكواد (Zero Code Truncation)**. يهدف هذا الملف إلى تمكين أي مطوّر من إعادة بناء وإدراج ميزة **Analytics BI Dashboard** تفاعلية ممتدة تشمل 6 كيانات رئيسية (**Lead, Opportunity, Account, Contact, Email, Meeting**) على نظام **EspoCRM** وفق معايير **الطبقة الرابعة (Tier-4 Architecture)** مع توثيق كافّة الأخطاء والحلول الجذرية المعتمدة.
...
4. **توفير بيئة أداء متطورة ودون اعتمادات خارجية (Native Only):**
   - الاعتماد التام على Backbone/RequireJS/jQuery/Native SVG دون استخدام مكتبات خارجية أو CDN لضمان الاستقرار وسرعة التحميل.
...
│   └── views/analytics-dashboard/index.js            <-- [7] واجهة العرض الكاملة ومحرك البيانات الشامل (6 كيانات + Dropdown Filters + SVG)
...
### 5️⃣ ملف واجهة العرض ومحرك البيانات الكامل المطور (Native Client View - 100% Full Code)
...
define('custom:views/analytics-dashboard/index', ['view'], function (Dep) {
    return Dep.extend({
        ...
        exportCSV: function () {
            ...
            if (this.activeTab === 'all' || this.activeTab === 'Lead') {
                csv += '--- LEADS ---\n';
                csv += 'ID,Name,Status,Source,Industry,Opportunity Amount,Website,Email,Phone,Do Not Call,City,Country,Assigned User,Created Date\n';
                this.rawLeads.forEach(function (l) {
                    csv += [escapeCsv(l.id), escapeCsv(l.name), escapeCsv(l.status), escapeCsv(l.source), escapeCsv(l.industry), l.opportunityAmount || 0, escapeCsv(l.website), escapeCsv(l.emailAddress), escapeCsv(l.phoneNumber), l.doNotCall ? 'Yes' : 'No', escapeCsv(l.addressCity), escapeCsv(l.addressCountry), escapeCsv(l.assignedUserName), escapeCsv(l.createdAt)].join(',') + '\n';
                });
                csv += '\n';
            }
            ...
            if (this.activeTab === 'all' || this.activeTab === 'Opportunity') {
                csv += '--- OPPORTUNITIES ---\n';
                csv += 'ID,Name,Account,Stage,Amount,Probability,Lead Source,Close Date,Assigned User,Created Date\n';
                this.rawOpps.forEach(function (o) {
                    csv += [escapeCsv(o.id), escapeCsv(o.name), escapeCsv(o.accountName), escapeCsv(o.stage), o.amount || 0, (o.probability || 0) + '%', escapeCsv(o.leadSource), escapeCsv(o.closeDate), escapeCsv(o.assignedUserName), escapeCsv(o.createdAt)].join(',') + '\n';
                });
                csv += '\n';
            }
            ...
            if (this.activeTab === 'all' || this.activeTab === 'Account') {
                csv += '--- ACCOUNTS ---\n';
                csv += 'ID,Name,Type,Industry,SIC Code,Billing City,Billing State,Billing Country,Website,Email,Phone,Assigned User,Created Date\n';
                this.rawAccounts.forEach(function (a) {
                    csv += [escapeCsv(a.id), escapeCsv(a.name), escapeCsv(a.type), escapeCsv(a.industry), escapeCsv(a.sicCode), escapeCsv(a.billingAddressCity), escapeCsv(a.billingAddressState), escapeCsv(a.billingAddressCountry), escapeCsv(a.website), escapeCsv(a.emailAddress), escapeCsv(a.phoneNumber), escapeCsv(a.assignedUserName), escapeCsv(a.createdAt)].join(',') + '\n';
                });
                csv += '\n';
            }

            if (this.activeTab === 'all' || this.activeTab === 'Contact') {
                csv += '--- CONTACTS ---\n';
                csv += 'ID,Name,Account,Title/Role,Email,Phone,Do Not Call,City,Country,Assigned User,Created Date\n';
                this.rawContacts.forEach(function (c) {
                    csv += [escapeCsv(c.id), escapeCsv(c.name), escapeCsv(c.accountName), escapeCsv(c.title), escapeCsv(c.emailAddress), escapeCsv(c.phoneNumber), c.doNotCall ? 'Yes' : 'No', escapeCsv(c.addressCity), escapeCsv(c.addressCountry), escapeCsv(c.assignedUserName), escapeCsv(c.createdAt)].join(',') + '\n';
                });
                csv += '\n';
            }

            if (this.activeTab === 'all' || this.activeTab === 'Email') {
                csv += '--- EMAILS ---\n';
                csv += 'ID,Subject,Status,Date Sent,From,Is Replied,Is Read,Assigned User,Created Date\n';
                this.rawEmails.forEach(function (e) {
                    csv += [escapeCsv(e.id), escapeCsv(e.name), escapeCsv(e.status), escapeCsv(e.dateSent), escapeCsv(e.fromString), e.isReplied ? 'Yes' : 'No', e.isRead ? 'Yes' : 'No', escapeCsv(e.assignedUserName), escapeCsv(e.createdAt)].join(',') + '\n';
                });
                csv += '\n';
            }

            if (this.activeTab === 'all' || this.activeTab === 'Meeting') {
                csv += '--- MEETINGS ---\n';
                csv += 'ID,Name,Status,Date Start,Date End,Duration (s),Parent Name,Parent Type,Assigned User,Created Date\n';
                this.rawMeetings.forEach(function (m) {
                    csv += [escapeCsv(m.id), escapeCsv(m.name), escapeCsv(m.status), escapeCsv(m.dateStart), escapeCsv(m.dateEnd), m.duration || 0, escapeCsv(m.parentName), escapeCsv(m.parentType), escapeCsv(m.assignedUserName), escapeCsv(m.createdAt)].join(',') + '\n';
                });
                csv += '\n';
            }
            ...
            link.download = 'CodakCRM_BI_Export_' + (this.activeTab) + '_' + new Date().toISOString().substring(0,10) + '.csv';
            link.click();
        },
...
### 7️⃣ ملف الترجمات العالمية `Global.json`
...
## 📐 مواصفات التوسعة الفنية ونظام التصميم (Technical Architecture Specs)

### 1️⃣ التوسعة لـ 6 كيانات CRM رئيسية (Multi-Entity Integration)
1. **`Lead`**: تحليلات المصادر، الحالات، المبالغ المتوقعة، ونسبة التحويل (`Conversion Rate`).
2. **`Opportunity`**: خط الإيرادات المغلقة (`Closed Won Revenue`)، مراحل الفنل (Stages)، ومجمل المحفظة المرجحة (`Weighted Pipeline`).
3. **`Account`**: أنواع الحسابات (`Customer`, `Partner`, `Investor`)، والقطاعات، والتوزيع الجغرافي.
4. **`Contact`**: نسبة توفر البريد والفيسبوك/الهاتف، ونسبة حظر الاتصال (`Do Not Call Ratio`).
5. **`Email`**: حالة الرسائل (`Draft`, `Sent`, `Failed`)، نسبة القراءة (`Read Rate`)، ونسبة الردود (`Reply Rate`).
6. **`Meeting`**: حالة الاجتماعات (`Planned`, `Held`, `Not Held`)، الاجتماعات القادمة (`Upcoming`)، ومعدل المدة الزمنية بالدقائق.

### 2️⃣ معالجة الحقول غير المخزنة بالـ Database (`notStorable` REST API Fields)
عند جلب بيانات `Email` عبر REST API، يجب تجنب طلب الحقول الافتراضية مثل `subject` أو `hasAttachment` داخل параметр `&select=` لأنها حقول `notStorable` تسبّب أخطاء HTTP 400/500. البديل المعتمد هو استخدام الحقل المخزن `name` لعنوان الرسالة و `fromString` لمرسلها.

### 3️⃣ شريط الفلاتر المنسدلة الاحترافي (Enterprise Dropdown Control Toolbar)
تم تحويل الفلاتر من أزرار وحبات بدائية إلى عناصر اختيار منسدلة حديثة (`<select id="analytics-entity-select">` و `<select id="analytics-period-select">`) مزودة بأيقونات سهم مخصصة وحقول بحث متناسقة.

### 4️⃣ المعادلات الرياضية والتحليلية المعتمدة (Mathematical & Analytical Formulas)
1. **نسبة تحويل الليدات (Lead Conversion Rate):**
   $$\text{Conversion Rate} = \left( \frac{\text{Converted Leads}}{\text{Total Leads}} \right) \times 100$$
2. **المحفظة المرجحة (Weighted Pipeline Value):**
   $$\text{Weighted Pipeline} = \sum \left( \text{Amount}_i \times \frac{\text{Probability}_i}{100} \right)$$
3. **معدل الفوز بالصفقات (Opportunity Win Rate):**
   $$\text{Win Rate} = \left( \frac{\text{Closed Won Deals}}{\text{Total Opportunities}} \right) \times 100$$
4. **متوسط حجم الصفقة (Average Deal Size):**
   $$\text{Average Deal Size} = \frac{\text{Total Closed Won Revenue}}{\text{Closed Won Deals Count}}$$
5. **نسبة الردود على الإيميلات (Email Reply Rate):**
   $$\text{Reply Rate} = \left( \frac{\text{Replied Emails Count}}{\text{Total Emails Count}} \right) \times 100$$
6. **معدل مدة الاجتماعات بالدقائق (Average Meeting Duration):**
   $$\text{Avg Duration (mins)} = \operatorname{round}\left( \frac{\sum \text{Duration (seconds)}}{\text{Total Meetings Count} \times 60} \right)$$

---

## 🚨 سجل التشخيص الهندسي وكافة الأخطاء الشائعة والحلول الجذرية (Troubleshooting Log)
...
### ❌ المشكلة 4: قراءة `0 Leads` أو عدم التحديث (Incorrect REST API Path)
...
### ❌ المشكلة 6: قص نصوص ومبالغ رسم الـ SVG البياني (SVG Area Chart Text Clipping)
- **السبب العلمي:** رسم المنحنى مباشرة على أطراف الـ ViewBox (0,0) بدون هامش داخلي مما يقطع الأرقام والتواريخ عند الأطراف.
- **الحل:** إضافة padding داخلي حسابي (`padL = 60`, `padR = 24`, `padT = 20`, `padB = 24`) واستخدام محاذاة ذكية `text-anchor="start|middle|end"`.

---

### ❌ المشكلة 7: تعارض الفلتر التفاعلي للـ Drill-down مع الكيانات (Category Filter Scope Leakage)
- **السبب العلمي:** عند تصفية فئة معينة (مثل `Status = Assigned`) في تبويب Leads، كان الفلتر يمتد لفلترة بقية الكيانات كـ Opportunity و Account بشكل خاطئ.
- **الحل:** تخزين الكيان المصدر داخل الفلتر `activeCategoryFilter = { entity: this.activeTab, field: field, value: val }` وتطبيقه على الكيان المعني حصراً.

---

### ❌ المشكلة 8: خطأ الفروق الزمنية في فلتر "Today" (UTC vs Local Timezone Shift)
- **السبب العلمي:** استخدام `toISOString().substring(0,10)` يقيس التاريخ بتوقيت جرينتش UTC، مما يتسبب في إزاحة اليوم عند فترات المساء.
- **الحل:** إنشاء دالة `getLocalDateStr()` تحسب التاريخ المحلي للعميل بترميز `YYYY-MM-DD`.

---

### ❌ المشكلة 9: تصفية القيمة المنطقية (Boolean Drill-down) تفرغ الجدول بالكامل (Email/Contact Ratio Drill-down Bug)
- **السبب العلمي:** مقارنة السلسلة النصية المعروضة "Replied" أو "Do Not Call" مع قيمة الـ Boolean في قاعدة البيانات (`true`/`false`) باستخدام `String(item['isReplied']) === String('Replied')` مما ينتج عنه دائماً `false`.
- **الحل:** فصل الرسم البياني للنسب التراكمية (Ratio Charts) عبر `renderCustomRatioChart` بدون ربط أحداث الـ Drill-down أو تحويل القيم بصراحة عبر `customEvaluator`.

---

### ❌ المشكلة 10: خطأ في استعلام الـ REST API لكيان الإيميل بسبب الحقول غير المتوفرة بالداتابيز (`notStorable` REST API Fields)
- **السبب العلمي:** تضمين حقول مثل `subject`, `from`, `hasAttachment` في параметр `&select=` بالرغم من أنها حقول `notStorable` تحسب وقت العرض فقط في EspoCRM، مما يسبّب إما إرجاع قيم فارغة أو حدوث استثناء في الـ REST Controller.
- **الحل:** الاعتماد صراحة على الحقل المخزن `name` لعنوان الإيميل و `fromString` لعنوان المرسل فقط داخل استعلام `select`.

---

### ❌ المشكلة 11: عدم استجابة زِر العرض "View" في الجداول أو إعادة تحميل الصفحة كلياً (SPA Router Navigation Failure)
- **السبب العلمي:** الاعتماد على الروابط التقليدية `<a href="#Lead/view/...">` دون اعتراض الحدث بواسطة Backbone Router، مما يسبب أحياناً عجز المتصفح عن فتح السجل داخل تطبيق الـ Single Page Application (SPA).
- **الحل:** إضافة معالج أحداث مخصص `onViewRecordClick` يربط النقر على زر `.btn-view-record` ويستخدم `window.Backbone.history.navigate(route, { trigger: true })`.

---

### ❌ المشكلة 12: فشل تطبيق فلتر الفترات الزمنية (`Period Filter`) على كيانات Meeting و Email
- **السبب العلمي:** تطبيق الفلتر الزمني بناءً على حقل الافتراضي `createdAt` لكل الكيانات، بينما تاريخ الفعالية الحقيقي للـ Meeting هو `dateStart` وللـ Email هو `dateSent`.
- **الحل:** تخصيص اسم حقل التاريخ المستهدف لكل كيان داخل دالة `filterByDate(list, dateProp)` ليصبح `dateSent` للإيميلات و `dateStart` للاجتماعات.

---

### ❌ المشكلة 13: ظهور شاشة بيضاء بدون محتوى عند التوجه لصفحة التفاصيل (Incomplete ClientDefs Metadata Schema)
...
```
ثم عمل **`Ctrl + Shift + R`** (Hard Refresh) in المتصفح.

---

## 🎨 الإضافات والتحديثات الجديدة الشاملة (System-Wide Design & Functional Overhauls)

بالإضافة إلى لوحة التحليلات (Analytics Dashboard)، تم إجراء تحديثات شاملة لتصميم وتكامل كافة كيانات النظام وفق أعلى معايير الـ Tier-4:

### 1️⃣ تطوير التبويب النشط في القائمة الجانبية (Sidebar Active Indicator)
- **شريط الإضاءة المباشر:** تم إضافة مؤشر إضاءة عمودي بلون الهوية السماوي (`border-left: 4px solid #00a4c8`) مع خلفية متدرجة ذات شفافية ناعمة للتبويب النشط حالياً في القائمة الجانبية داخل [client/custom/css/custom-brand.css](file:///d:/laragon/www/EspoCRM-10.0.3/client/custom/css/custom-brand.css).
- **إزالة النقطة النابضة:** تم إزالة الـ Pulse Dot الزائدة (`::after`) لتنقية المظهر البصري.

### 2️⃣ التوحيد القياسي لأيقونات كافّة الكيانات (Core Entities ClientDefs Icons)
تم تحديث كافّة ملفات الـ `clientDefs` للكيانات الأساسية تحت `custom/Espo/Custom/Resources/metadata/clientDefs/` بأيقونات FontAwesome احترافية وموحدة:
- **Lead:** `"iconClass": "fas fa-user-tag"`
- **Opportunity:** `"iconClass": "fas fa-briefcase"`
- **Account:** `"iconClass": "fas fa-building"`
- **Contact:** `"iconClass": "fas fa-address-book"`
- **Meeting:** `"iconClass": "fas fa-calendar-alt"`
- **Call:** `"iconClass": "fas fa-phone-volume"`
- **Task:** `"iconClass": "fas fa-list-check"`
- **Case:** `"iconClass": "fas fa-folder-open"`
- **Campaign:** `"iconClass": "fas fa-bullhorn"`
- **Email:** `"iconClass": "fas fa-envelope"`
- **Document:** `"iconClass": "fas fa-file-lines"`

### 3️⃣ إعادة البناء الهيكلي والجمالي لكافّة الجداول والتصفح (Ultra-Luxury Data Grid & Pagination)
- **كبسولة العداد الزجاجية (Glassmorphic Pagination Badge):** تحويل شريط `1-1 / 1 < >` إلى كبسولة زجاجية عائمة بظلال ناعمة وتأثير ضبابي (`backdrop-filter: blur(8px)`) محاذاة بدقة أعلى اليمين.
- **تدرج رؤوس الجداول (Gradient Table Header):** تطبيق تدرج رمادي ناعم (`linear-gradient(180deg, #f8fafc 0%, #f1f5f9 100%)`) مع خطوط سفلية دقيقة ونصوص كابيتال جريئة داكنة (`#1e293b`).
- **أنميشن التمرير والتوهج (Glowing Row Hover & Slide):** عند التمرير على أي صف، يظهر تدرج ضوئي ناعم مع مؤشر إضاءة متوهج على اليسار (`border-left: 4px solid #00a4c8`) وتأثير انزلاق ناعم للروابط (`transform: translateX(3px)`).
- **شارات البيانات الذكية (Smart Email & Phone Badges):** تمييز الإيميلات بلون أزرق ناصع (`#0284c7`) وأرقام الهواتف بنمط واضع ومقروء (`#334155`).
- **بطاقة الصفحة الفارغة (Styled Empty State):** تحسين مظهر الجداول التي لا تحتوي على بيانات ببطاقات توضيحية عائمة بنمط منقط (`border: 1px dashed #cbd5e1`).

---

## ❌ المشاكل الإضافية والحلول الجذرية المعتمدة (Additional Solved Issues Manual)

---

### ❌ المشكلة 14: كسر رابط تفاصيل الفرصة التجارية (Opportunity Detail View Route Breakage / RequireJS 404 Error)
- **السبب العلمي:** كيان الفرصة التجارية (`Opportunity`) في EspoCRM يعتمد مسار View مخصص ينتهي بـ `crm:views/opportunity/detail` دون المجلد الفرعي `record/detail` المستخدم في بقية الكيانات (مثل `crm:views/lead/record/detail`). محاولة ربط `crm:views/opportunity/record/detail` تسببت في خطأ RequireJS 404 لشاشة التفاصيل.
- **الحل:** إنشاء المعالج المخصص [client/custom/src/views/opportunity/detail.js](file:///d:/laragon/www/EspoCRM-10.0.3/client/custom/src/views/opportunity/detail.js) كامتداد صريح لـ `crm:views/opportunity/detail` وتحديث تعيين الخريطة في [Opportunity.json](file:///d:/laragon/www/EspoCRM-10.0.3/custom/Espo/Custom/Resources/metadata/clientDefs/Opportunity.json):
  ```json
  {
      "controller": "controllers/record",
      "modelDefaultsPreparator": "crm:handlers/opportunity/defaults-preparator",
...
### ❌ المشكلة 15: تشوه ومحاذاة شريط التصفح والعداد أعلى الجداول واقتطاعه في أعلى الصفحة (List Pagination Flexbox & Cutoff Fix)
- **السبب العلمي:** الاعتماد على كلاسات غير دقيقة مثل `table.list` واختلاط `float: right` مع حاوية `display: flex` في الـ CSS الخارجي، مما تسبب في خروج كبسولة العداد `.btn-group.pagination` فوق حدود الصفحة المتاحة واقتطاع الجزء العلوي منها.
- **الحل:** استهداف الكلاسات الدقيقة لقوالب Backbone في EspoCRM 10 (`.list-buttons-container` و `.btn-group.pagination`) وإلغاء الـ `float` واستبداله بالـ Flexbox المباشر لمنع أي اقتطاع بحدود الصفحة:
  ```css
...
### ❌ المشكلة 16: تداخل أسهم الترتيب مع نصوص هيدر الأعمدة (Sort Caret Text Overlap)
- **السبب العلمي:** طباعة أسهم الترتيب (`.fa-chevron-down` و `.fa-chevron-up`) مباشرة داخل رابط الهيدر بدون ضبط الـ Inline Flex Alignment، مما جعل السهم يلتصق أو ينزل فوق الحرف الأخير من اسم العمود (مثل ظهور `NĀME`).
- **الحل:** استهداف `th.field-header-cell a` بخصائص `display: inline-flex; align-items: center; gap: 6px;` مع منح السهم `margin-left: 4px`.

---

### ❌ المشكلة 17: تشوه حواف الصفوف وأزرار الإجراءات في الزاوية السفلى للجداول (Bottom-Right Action Dropdown Clipping)
- **السبب العلمي:** تطبيق `overflow: hidden` على حاوية الجدول الرئيسية مع تفعيل نموذج `border-collapse: collapse` الافتراضي في Bootstrap، مما تسبب في اقتطاع أزرار القائمة المنبثقة السفلى وخروج مربع التحديد.
- **الحل:** استخدام `border-collapse: separate; border-spacing: 0;` على `div.list > table.table` لضمان حماية حواف الزوايا مع الحفاظ على مرونة التنسيق.

---

### ❌ المشكلة 18: استعادة الفلاتر وحل تعارض البحث في الصفحات الرئيسية (List Search & Filter Restoration)
- **السبب العلمي:** تعديل أنماط `.input-group` بطريقة عامة تسببت في إخفاء أو محاذاة خاطئة لأزرار البحث والـ Filter Dropdown في الهيدر العلوي لصفحات القوائم.
- **الحل:** إعادة صياغة قواعد `.search-container` و `.input-group-btn > .btn` في [client/custom/css/custom-brand.css](file:///d:/laragon/www/EspoCRM-10.0.3/client/custom/css/custom-brand.css) لتضمن إغلاق وربط عناصر الإدخال بشكل متناسق مع الحفاظ على القوائم المنسدلة للبحث.

---

### ❌ المشكلة 19: حل مشكلة اعمدة إكسل العشوائية (Column1..Column13) والترميز العلمي للهواتف وتجاوز التواريخ
- **السبب العلمي:**
  1. عند وجود سطر ترويسة رئيسية بحقل واحد في بداية التصدير (مثل `--- LEADS ---`), يقوم إكسل بتسمية بقية الأعمدة تلقائياً `Column1`, `Column2`... `Column13`.
  2. أرقام الهواتف الطويلة يحولها إكسل تلقائياً إلى صيغ علمية أسّية (مثل `1.1566E+10`).
  3. التواريخ الطويلة تتجاوز عرض الخلية فتظهر في إكسل كـ `##########`.
- **الحل الشامل والنهائي:**
  1. **التصدير المباشر المخصص للتبويب (Single Tab Direct Export):** عند اختيار تبويب معين (مثل Leads)، يبدأ ملف الـ CSV فوراً من السطر الأول برؤوس الأعمدة الحقيقية (`ID,Name,Status,Source...`), مما يلغي تماماً اختراع إكسل لأعمدة `Column1` و `Column2`.
  2. **حشو ترويسات Overview بالحقول الفارغة (Padded Header Rows):** عند تصدير شاشة التجميع (Overview)، تم حشو أسطر العناوين بحقول فارغة `makeTitleRow` لتطابق تماماً عدد أعمدة الجدول التالي لها.
